...
import com.example.tiktokandroid.core.utils.FileUtils
...
fun VideoPlayer(
    ...
    LaunchedEffect(key1 = video.videoUrl) {
        withContext(Dispatchers.IO) {
            val bm = try {
                if (video.videoUrl.startsWith("http")) {
                    FileUtils.extractThumbnailFromUrl(video.videoUrl, 1)
                } else {
                    FileUtils.extractThumbnailFromAsset(context, "videos/${video.videoUrl}", 1)
                }
            } catch (e: Exception) {
                e.printStackTrace()
                null
